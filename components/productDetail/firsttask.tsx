...
const Divider = () => (
  <hr className="my-6 border-0 border-t border-neutral-200" />
);
...
const variant = useMemo(
  ...
  return (
    <main className="mx-auto max-w-[1250px] px-4 py-10 lg:py-12">
     <div className="grid grid-cols-1 gap-10 lg:grid-cols-[600px_minmax(0,1fr)] lg:gap-14">
        <div className="grid grid-cols-[80px_minmax(0,1fr)] gap-5">
        <div className="sticky top-16 ml-3 flex max-h-[calc(100vh-140px)] flex-col gap-2 overflow-y-auto pr-1">
            {variant.images.map((src, i) => (
              <button
                ...
                className={`aspect-[3/4] w-[64px] h-[110px] rounded-lg bg-neutral-80 p-0.5 flex items-center justify-center ${
                  i === imgIndex ? "border-2 border-black" : "border border-transparent hover:border-neutral-300"
                }`}
              >
                <img
      ...
      <SwiperSlide key={src + i} className="h-full">
        ...
        <div>
          ...
          <p className="text-[12px] font-medium uppercase tracking-wider text-neutral-600">
            Colour:{" "}
            <span className="normal-case font-semibold">{variant.color}</span>
          </p>
            ...
            {product.variants.map((v) => {
              ...
              return (
                <button
                  ...
                  title={v.color}
                  className={`h-20 w-15 overflow-hidden rounded-md p-0.5 ${
            ...
            >
              {variant.sizes.map((s) => (
                <RadioGroup.Item
                  key={s}
                  value={s}
                  className="h-10 rounded-md px-3 text-sm shadow-[inset_0_0_0_1px_rgba(0,0,0,0.12)] data-[state=checked]:bg-black data-[state=checked]:text-white hover:bg-neutral-50"
                >
                  {s}
                </RadioGroup.Item>
              ))}
          ...
          <div className="mt-6 flex items-start gap-3 text-sm text-neutral-700">
            ...
            <p>
              Add complimentary gift wrap and handwritten note in cart or
              checkout
            </p>
          </div>
          ...
          <p className="mt-3 text-center text-[12px] uppercase tracking-wider text-neutral-500">
            Made in Italy &nbsp;–&nbsp; Free Returns &amp; Exchanges
            &nbsp;–&nbsp; Secure Payments
          </p>
          <Divider />
