                        ...
                        <div className="flex gap-4">
                            <div className="navigation-item-submenu-content flex-1">
                                {item.items?.map((subItem) => (
                                    <div className="navigation-item-submenu-item flex flex-col gap-2" key={subItem.name}>
                                        <a href={subItem.href} className="hover:underline font-semibold text-sm uppercase">{subItem.name}</a>
                                        {subItem?.items?.map((subItem123) => (
                                            <a key={subItem123.name} className="hover:underline text-sm uppercase">{subItem123.name}</a>
                                        ))}
                                    </div>
                                ))}
                            </div>
